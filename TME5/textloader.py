...
def pad_collate_fn(samples: List[List[int]]):
    ...
    end = torch.tensor(EOS_IX).view(1)
    seq = [torch.cat((x, end)) for x in samples]
    return torch.nn.utils.rnn.pad_sequence(seq, batch_first=False)
...
if __name__ == "__main__":
    ...
    # Longueur maximum
    assert (data.shape[0],data.shape[1]) == (7, 3)
    print("Shape ok")
    # e dans les deux cas
